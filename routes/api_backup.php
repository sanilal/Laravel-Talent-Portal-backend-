...
Route::prefix('v1')->group(function () {
     ...
     */
    Route::get('/public/categories/{categoryId}/subcategories', [CategoryController::class, 'subcategories'])
        ->name('public.categories.subcategories');

     /**
     * GET /api/v1/public/subcategories/{id}
     * Get a single subcategory
     */
    Route::get('/public/subcategories/{id}', [CategoryController::class, 'showSubcategory'])
        ->name('public.subcategories.show');
     ...
     * 15 = Gender
     */
    Route::get('/public/dropdown-list', [DropdownController::class, 'index'])
        ->name('public.dropdown.index');
     ...
     * Body: { "types": [1, 2, 3] }
     */
    Route::post('/public/dropdown-list/multiple', [DropdownController::class, 'multiple'])
        ->name('public.dropdown.multiple');

    /**
     * GET /api/v1/public/dropdown-list/all
     * Get all dropdown types with values
     */
    Route::get('/public/dropdown-list/all', [DropdownController::class, 'all'])
        ->name('public.dropdown.all');
    ...
    Route::get('/public/countries', [CountryController::class, 'index'])
        ->name('public.countries.index');

    /**
     * GET /api/v1/public/countries/{id}
     * Get a single country with its states
     */
    Route::get('/public/countries/{id}', [CountryController::class, 'show'])
        ->name('public.countries.show');
     ...
     */
    Route::get('/public/states', [CountryController::class, 'states'])
     ...
     */
    Route::get('/public/countries/search', [CountryController::class, 'search'])
        ->name('public.countries.search');
     ...
     * Get all project types (Feature Film, Short Film, etc.)
     * Compatible with: yourmoca.com/api/getPostRequestTypes
     */
    Route::get('/public/project-types', [ProjectTypeController::class, 'index'])
        ->name('public.project-types.index');

    /**
     * GET /api/v1/public/project-types/{id}
     * Get a single project type
     */
    Route::get('/public/project-types/{id}', [ProjectTypeController::class, 'show'])
        ->name('public.project-types.show');

    // ============================================
    // PROFILES (Talent Listings with Filters)
    // ============================================
     ...
     * - height: Filter by height dropdown value
     * - weight: Filter by weight range dropdown value
     * - skinTone: Filter by skin tone dropdown value
     * - ageMin: Minimum age
     * - ageMax: Maximum age
     * - gender: Filter by gender
     * - page: Page number (default: 1)
     * - limit: Results per page (default: 15)
     * - sortBy: created_at, profile_views, average_rating
     * - sortOrder: asc or desc
     * 
     * Example:
     * /api/v1/public/profiles?categoryId=xxx&subcategoryId=yyy&country=50&state=3&page=1&limit=15
     */
        Route::get('/public/profiles', [ProfileController::class, 'index'])
         ...
         */
        Route::get('/public/profiles/{id}', [ProfileController::class, 'show'])
 ...
 * ======================
 * 
 * This API structure is designed to be compatible with yourmoca.com
 * while maintaining Laravel best practices.
 * 
 * MAIN DIFFERENCES FROM YOURMOCA:
 * 1. Uses UUID for categories/subcategories (not integer IDs)
 * 2. More RESTful endpoint structure
 ...
 * 
 * DROPDOWN TYPES:
 * Type 1  = Heights (4'6" to 7'0"+)
 * Type 2  = Skin Tones (Fair, Light, Medium, etc.)
 * Type 3  = Weight Ranges (20-25 Kg to 195-200 Kg)
 * Type 4  = Age Ranges (0-5 to 100+)
 * Type 5  = Vehicle Types (Car, Van, Bus, etc.)
 * Type 6  = Service Types (Shuttle, Car Rental, etc.)
 * Type 7  = Event Types (Wedding, Corporate, etc.)
 * Type 8  = Budget Ranges (0-10000 to 500000+)
 * Type 9  = Eye Colors (Brown, Blue, Green, etc.)
 * Type 10 = Hair Colors (Black, Brown, Blonde, etc.)
 * Type 11 = Body Types (Slim, Athletic, etc.)
 * Type 12 = Vocal Ranges (Soprano, Tenor, etc.)
 * Type 13 = Experience Levels (Beginner to Professional)
 * Type 14 = Language Proficiency (Basic to Native)
 * Type 15 = Gender (Male, Female, etc.)
 * 
 * PROJECT TYPES:
 * - Feature Film
 * - Short Film
