...
use App\Services\TalentAttributesService;
...
class TalentProfileController extends Controller
{
    ...
    {
        $this->attributesService = new TalentAttributesService();
    }

    /**
     * Get talent profile
     * 
     * UPDATED: Now includes profile_attributes and skill_attributes
     */
    public function show(Request $request)
    {
        ...
        // NEW: Add profile-level and skill-level attributes
        if ($user->talentProfile) {
            try {
                $profile['profile_attributes'] = $this->attributesService->getProfileLevelAttributes($user->talentProfile);
                $profile['skill_attributes'] = $this->attributesService->getSkillLevelAttributes($user->talentProfile);
            } catch (\Exception $e) {
                Log::warning('Error loading attributes: ' . $e->getMessage());
                $profile['profile_attributes'] = [];
                $profile['skill_attributes'] = [];
            }
        }
 ...
 */
    ...
    // ========================================================================
    // NEW METHODS: PROFILE-LEVEL ATTRIBUTES
    // ========================================================================

    /**
     * Get profile-level attributes (physical attributes)
     * 
     * GET /api/v1/talent/profile/attributes
     */
    public function getAttributes(Request $request)
    {
        try {
            ...
            $attributes = $this->attributesService->getProfileLevelAttributes($profile);
            ...
            Log::error('Error fetching profile attributes: ' . $e->getMessage());
            return response()->json([
    ...
    /**
     * Update profile-level attributes (physical attributes)
     * 
     * PUT /api/v1/talent/profile/attributes
     * 
     * Body:
     * {
     *   "height": "5_10",
    ...
    public function updateAttributes(Request $request)
    {
        try {
            ...
            // Remove null values
            $attributes = array_filter($validated, function($value) {
                return $value !== null;
            });

            // Update attributes
            $this->attributesService->setProfileAttributes($profile->id, $attributes);

            // Get updated attributes
            $updatedAttributes = $this->attributesService->getProfileLevelAttributes($profile);
            ...
            Log::error('Error updating profile attributes: ' . $e->getMessage());
            return response()->json([
     ...
     * (Can be public - just definitions, no data)
     */
    public function getAvailableAttributes(Request $request)
    {
        try {
            $definitions = $this->attributesService->getProfileLevelAttributeDefinitions();
            ...
            return response()->json([
                'message' => 'Error fetching attribute definitions',
                'error' => $e->getMessage()
            ], 500);
        }
    }
    ...
    /**
     * Delete a specific profile-level attribute
     * 
     * DELETE /api/v1/talent/profile/attributes/{fieldName}
     */
    public function deleteAttribute(Request $request, $fieldName)
    {
        try {
            $user = $request->user();
            $profile = $user->talentProfile;
            ...
            $deleted = $this->attributesService->deleteProfileAttribute($profile->id, $fieldName);
            ...
            Log::error('Error deleting profile attribute: ' . $e->getMessage());
            return response()->json([
